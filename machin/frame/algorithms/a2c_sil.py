...
class A2C_SIL(A2C):
    ...
    def update_sil(self, **__):
        ...
        self.actor.train()
        self.critic.train()
        if self.replay_buffer_sil.size() > 0:
            for _ in range(self.sil_update_times):
                ...
                act_policy_loss = act_policy_loss.mean() * self.sil_actor_loss_weight
                ...
                sil_value_loss = sil_value_loss.pow(2).sum() * self.sil_value_loss_weight

                # Update critic network
                self.critic.zero_grad()
                self._backward(sil_value_loss)
